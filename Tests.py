...
def setup_shear_wave(file_path: str) -> dict | Shearwave:
    ...
    plate_shear = Plate(
        thickness, 6060, 3230, rayleigh_wave_velocity=None, name="Titanium"
    )
...
def setup_lamb_wave(file_path) -> dict | Lambwave:
    ...
    plate_lamb = Plate(
        thickness, 5770, 3050, rayleigh_wave_velocity=None, name="Magnesium"
    )
    ...
    for mode, (freqs, vels) in data.items():
        if not legend_added["disperse_software"]:
            plt.plot(
                freqs,
                vels,
                label="Disperse Software",
                color=color_wave_dispersion_software,
            )
        ...
        for v in velocites:
            if mode in v:
                sim_freqs, sim_vels = (
                    zip(*[(fd, vel) for fd, vel, *_ in v[mode]])
                    if type == "Phase"
                ...
                if not legend_added["wave_dispersion_software"]:
                    plt.plot(
                        sim_freqs,
                        sim_vels,
                        label="WaveDispersion Software",
                        color=color_my_software,
                    )
...
if __name__ == "__main__":
    ...
    plot_data(
        ...
        "validation/Titanium_Shear_Phase.png",
    )
    plot_show()
    ...
    plot_data(
        ...
        "validation/Magnesium_Lamb_Phase.png",
    )
    plot_show()
    ...
    plot_data(
        ...
        "validation/Titanium_Shear_Group.png",
    )
    plot_show()
    ...
    plot_data(
        ...
        "validation/Magnesium_Lamb_Group.png",
    )
    plot_show()
