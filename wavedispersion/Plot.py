...
class Plot:
    ...
    symmetric_style: dict = field(
        default_factory=lambda: {"color": "green", "linestyle": "-"}
    )
    antisymmetric_style: dict = field(
        default_factory=lambda: {"color": "purple", "linestyle": "--"}
    )
    torsional_style: dict = field(
        default_factory=lambda: {"color": "green", "linestyle": "-"}
    )
    longitudinal_style: dict = field(
        default_factory=lambda: {"color": "purple", "linestyle": "--"}
    )
    flexural_style: dict = field(
        default_factory=lambda: {"color": "orange", "linestyle": "-"}
    )
    dashed_line_style: dict = field(
        default_factory=lambda: {"color": "black", "linestyle": "--", "linewidth": 0.5}
    )
    continuous_line_style: dict = field(
        default_factory=lambda: {"color": "black", "linestyle": "-", "linewidth": 0.75}
    )
    in_plane_style: dict = field(
    ...
    velocity_style: dict = field(
        default_factory=lambda: {"color": "black", "va": "center"}
    )
    ...
    def _find_max_value(self, index: int) -> float:
        ...
        """
        max_value_sym = max(
            max([point[index] for point in values])
            for values in self.wave.velocites_symmetric.values()
        )
        max_value_anti = max(
            max([point[index] for point in values])
            for values in self.wave.velocites_antisymmetric.values()
        )
    ...
    def _add_cutoff_frequencies(self, mode: str, max_value: float, plot_type: str):
        ...
        Returns:
            None
        """
        arrow_y, arrow_dir, arrow_s = (
            (max_value, "top", r"$\downarrow$")
            if plot_type == "Phase"
            else (0, "bottom", r"$\uparrow$")
        )
        ...
        if isinstance(self.wave, Shearwave):
            ...
            self._draw_arrow(
                {"x": arrow_x, "y": arrow_y, "dir": arrow_dir, "s": arrow_s}
            )

        else:
            n = int(mode[2:]) + 1
            arrow_x = (
                n * self.wave.velocities_dict["C_S"]
                if mode.startswith("S")
                else n * self.wave.velocities_dict["C_L"]
            )
            self._draw_arrow(
                {"x": arrow_x, "y": arrow_y, "dir": arrow_dir, "s": arrow_s}
            )
            ...
            if n % 2 != 0:
                arrow_x = (
                    n * self.wave.velocities_dict["C_L"] / 2
                    if mode.startswith("S")
                    else n * self.wave.velocities_dict["C_S"] / 2
                )
                self._draw_arrow(
                    {"x": arrow_x, "y": arrow_y, "dir": arrow_dir, "s": arrow_s}
                )
    ...
    def _plot_velocity(self, plot_type: str):
        ...
        """
        title = (
            f"{plot_type} Velocity"
            if plot_type != "Wavenumber"
            else f'{plot_type[:4] + " " + plot_type[4:]}'
        )
        ...
        mode_mapping = {
            "symmetric": [
                (self.wave.velocites_symmetric, symmetric_lines, self.symmetric_style)
            ],
            "antisymmetric": [
                (
                    self.wave.velocites_antisymmetric,
                    antisymmetric_lines,
                    self.antisymmetric_style,
                )
            ],
            "both": [
                (self.wave.velocites_symmetric, symmetric_lines, self.symmetric_style),
        ...
        values, labels = (
            (
                [
                    mode_mapping[self.mode_type][0][1][0],
                    mode_mapping[self.mode_type][1][1][0],
                ],
                ["Symmetric", "Antisymmetric"],
            )
            if self.mode_type == "both"
            else ([mode_mapping[self.mode_type][0][1][0]], [self.mode_type])
        )
        ...
        (
            ...
            if plot_type != "Wavenumber"
            else plt.ylabel("Wavenumber (1/m)")
        )
        ...
        if self.wave.structure_result:
            if self.wave.rows == 1 and self.wave.columns == 1:
                fig, axes = plt.subplots(
                    1, 1, figsize=(10, 6), num=f"{title[:4] + ' ' + title[4:]}"
                )
                axes = [axes]
            else:
                fig, axes = plt.subplots(
            ...
            for i, key in enumerate(self.wave.structure_result.keys()):
                ...
                ax.axvline(
                    0, **self.continuous_line_style, ymin=0 + 0.05, ymax=1 - 0.05
                )
                ...
                ax.set_yticks(
                    [
                        -self.wave.material.half_thickness,
                        0,
                        self.wave.material.half_thickness,
                    ]
                )
                ...
                ax.text(
                    ax.get_xlim()[1] / 2,
                    self.wave.material.half_thickness / 5,
                    "u, w" if isinstance(self.wave, Lambwave) else "u",
                    ha="center",
                    va="center",
                )
            ...
            plt.tight_layout
            mode = (
                "SH_" + str(self.wave.get_converted_mode(self.wave.structure_mode))
                if isinstance(self.wave, Shearwave)
    ...
    def save_plots(
        self, format: str = "png", transparent: bool = False, **kwargs
    ) -> list[str]:
        """
        Save plots in specified format.
    ...
    def save_txt_results(self, date=False) -> str:
        ...
        wave_type_map = {Lambwave: "Lambwaves", Shearwave: "Shearwaves"}
